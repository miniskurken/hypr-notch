...
pub fn fill_canvas_with_rounded_corners(
    ...
    // First, fill entire canvas with the background color
    // This ensures no transparent pixels remain
    ...
    for y in 0..height as i32 {
        for x in 0..width as i32 {
            ...
            // Check if this pixel is in the bottom rounded corner areas
            let in_rounded_area = if y > height as i32 - radius {
                // Bottom left corner
                if x < radius {
                    let dx = radius - x;
                    let dy = y - (height as i32 - radius);
                    // Use >= instead of > to be more aggressive with transparency
